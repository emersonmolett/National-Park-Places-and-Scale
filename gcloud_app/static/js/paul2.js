...
// 	NPSplaceRandomLatLong = [NPSData[0].latitude, NPSData[0].longitude];
// })
...
function buildMap(selectedparkID) {
	...
	// }
	// if (mapdiv.html().length >10){
	// 	map.off()
	// 	map.remove()
	// 	mapdiv.html("")
	// }
	//map-wrapper and map
	const mapWrp = document.querySelector('.mapwrapper');
...
function updateMap(selectedparkID) {
	d3.json("/natparks").then(function (NPSData) {
		NPSplaceRandomLatLong = [NPSData[selectedparkID].latitude, NPSData[selectedparkID].longitude];
	})
}